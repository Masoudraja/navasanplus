...
class DiscountMetaBoxes {
  ...
  public function render_discount_box($post) {
    ...
    ?>
        <p>
            <label for="mnsnp_discount_profit_percentage"><?php _e(
              'Discount on Profit (%)',
              'mns-navasan-plus',
            ); ?></label>
            <input type="number" step="1" name="mnsnp_discount[profit_percentage]" id="mnsnp_discount_profit_percentage" value="<?php echo esc_attr(
              $fields['profit_percentage'],
            ); ?>" class="widefat">
        </p>
        <p>
            <label for="mnsnp_discount_profit_fixed"><?php _e(
              'Discount on Profit (Fixed)',
              'mns-navasan-plus',
            ); ?></label>
            <input type="number" step="1" name="mnsnp_discount[profit_fixed]" id="mnsnp_discount_profit_fixed" value="<?php echo esc_attr(
              $fields['profit_fixed'],
            ); ?>" class="widefat">
        </p>
        <p>
            <label for="mnsnp_discount_charge_percentage"><?php _e(
              'Discount on Charge (%)',
              'mns-navasan-plus',
            ); ?></label>
            <input type="number" step="1" name="mnsnp_discount[charge_percentage]" id="mnsnp_discount_charge_percentage" value="<?php echo esc_attr(
              $fields['charge_percentage'],
            ); ?>" class="widefat">
        </p>
        <p>
            <label for="mnsnp_discount_charge_fixed"><?php _e(
              'Discount on Charge (Fixed)',
              'mns-navasan-plus',
            ); ?></label>
            <input type="number" step="1" name="mnsnp_discount[charge_fixed]" id="mnsnp_discount_charge_fixed" value="<?php echo esc_attr(
              $fields['charge_fixed'],
            ); ?>" class="widefat">
        </p>
        
        <?php do_action('mnsnp_discount_box_after_fields', $post); ?>
        <?php
  }

  public function save_discount_box($post_id) {
    if (
      !isset($_POST['_mnsnp_discount_nonce']) ||
      !wp_verify_nonce($_POST['_mnsnp_discount_nonce'], 'mnsnp_discount_box')
    ) {
      return;
    }
    ...
    // Save time fields
    do_action('mnsnp_save_discount_box', $post_id);
  }
}
