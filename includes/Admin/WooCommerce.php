...
final class WooCommerce {
  ...
  public function run(): void {
    ...
    add_filter(
      ...
      [$this, 'set_and_filter_dynamic_prices'],
      20,
      2,
    );
    add_filter(
    ...
    // Add filters for cart-specific price handling
    add_filter('woocommerce_cart_product_price', [$this, 'set_and_filter_dynamic_prices'], 20, 2);
    add_filter(
    ...
    // Add filter for checkout price handling
    add_filter(
      'woocommerce_checkout_product_price',
      [$this, 'set_and_filter_dynamic_prices'],
      20,
      2,
    );

    // Add filter to update prices when cart totals are calculated
    add_action(
      'woocommerce_calculate_totals',
      [$this, 'update_cart_prices_on_calculate_totals'],
      20,
      1,
    );

    // Add filter to handle mini-cart price updates
    add_filter('woocommerce_cart_item_price_html', [$this, 'update_mini_cart_item_price'], 20, 3);
    ...
    add_action(
      ...
      [$this, 'add_order_item_meta'],
      10,
      4,
    );
    add_filter(
      'woocommerce_order_item_display_meta_key',
      [$this, 'display_order_item_meta_key'],
      10,
      3,
    );
    add_filter(
      'woocommerce_order_item_display_meta_value',
      [$this, 'display_order_item_meta_value'],
      10,
      3,
    );
    ...
    // Woodmart theme compatibility - ensure prices are updated in AJAX requests
    add_filter('woocommerce_cart_item_price', [$this, 'update_cart_item_price_display'], 20, 3);
    add_filter(
      'woocommerce_cart_item_subtotal',
      [$this, 'update_cart_item_subtotal_display'],
      20,
      3,
    );

    // Refresh cart prices on specific actions
    add_action('woocommerce_cart_loaded_from_session', [$this, 'refresh_cart_prices']);
    add_action('woocommerce_check_cart_items', [$this, 'refresh_cart_prices']);

    // Add filter for order pay page price handling
    add_filter(
    ...
    add_filter('woocommerce_get_order_item_totals', [$this, 'update_order_pay_totals'], 20, 3);
  }
  ...
  /**
   * Update order totals on order pay page
   */
  public function update_order_pay_totals($total_rows, $order, $tax_display) {
    // Check if we're on the order pay page
    if (\MNS\NavasanPlus\is_order_pay_page()) {
      // Recalculate the order totals with current prices
      $new_total = 0;
        ...
        if ($product) {
          ...
          if (class_exists(PriceCalculator::class)) {
            ...
            if ($res !== null) {
              ...
              $new_total += $price_after * $quantity;
            }
          }
        }
      }
  ...
  /**
   * Update mini-cart item price display
   */
  public function update_mini_cart_item_price($price_html, $cart_item, $cart_item_key) {
    // Force recalculate the price for mini-cart items
    if (isset($cart_item['data']) && $cart_item['data'] instanceof \WC_Product) {
      $product = $cart_item['data'];
      $this->set_and_filter_dynamic_prices($product->get_price(), $product);
    }
    return $price_html;
  }

  /**
   * Update cart prices when WooCommerce calculates totals
   */
  public function update_cart_prices_on_calculate_totals($cart) {
  ...
  /**
   * Refresh all cart prices to ensure they're up to date
   */
  public function refresh_cart_prices(): void {
    // Only run this in frontend context
  ...
  /**
   * Update cart item price display for Woodmart compatibility
   */
  public function update_cart_item_price_display($price, $cart_item, $cart_item_key) {
    // Check if this is a Woodmart AJAX request or any AJAX request
  ...
  /**
   * Update cart item subtotal display for Woodmart compatibility
   */
  ...
  public function set_and_filter_dynamic_prices($price, $product) {
    ...
    // But allow reprocessing if this is an AJAX request to ensure fresh prices
    $is_ajax_request = defined('DOING_AJAX') && DOING_AJAX;
    if (
      isset(self::$calc_guard[$pid]) &&
      !$is_ajax_request &&
      !\MNS\NavasanPlus\is_order_pay_page()
    ) {
    ...
    try {
      ...
      // Only unset the guard for non-AJAX requests to allow reprocessing during AJAX
      // Also allow reprocessing on order pay page
      if (!$is_ajax_request && !\MNS\NavasanPlus\is_order_pay_page()) {
  ...
  private function product_meta_bool(
    \WC_Product $product,
    string $suffix,
    bool $default = false,
  ): bool {
  ...
  public function save_product_object(\WC_Product $product): void {
    ...
    foreach ($decimals as $key) {
      ...
      if (isset($_POST[$post_key])) {
        $product->update_meta_data(
          $db->full_meta_key($key),
          wc_format_decimal(wp_unslash($_POST[$post_key])),
        );
      }
    }
    ...
    if (
      isset($_POST['_mns_navasan_plus_formula_variables']) &&
      ...
      foreach ($_POST['_mns_navasan_plus_formula_variables'] as $fid => $codes) {
        ...
        foreach ((array) $codes as $code => $vals) {
          ...
          $vars[$fid][$code] = [
            'regular' => isset($vals['regular'])
              ? sanitize_text_field(wp_unslash($vals['regular']))
              : '',
            'sale' => isset($vals['sale']) ? sanitize_text_field(wp_unslash($vals['sale'])) : '',
          ];
        }
      }
  ...
  public function save_product_variation(int $variation_id, int $i): void {
    ...
    foreach (['dependence_type', 'rounding_type', 'rounding_side', 'profit_type'] as $key) {
      ...
      if ($raw !== null) {
        update_post_meta($variation_id, $db->full_meta_key($key), sanitize_text_field($raw));
      }
    }
    $raw = $this->vpost('_mns_navasan_plus_currency_id', $i);
    if ($raw !== null) {
      update_post_meta($variation_id, $db->full_meta_key('currency_id'), absint($raw));
    }
    $decimals = ['profit_value', 'rounding_value', 'ceil_price', 'floor_price'];
    foreach ($decimals as $key) {
      $base = "_mns_navasan_plus_{$key}";
      $raw = $this->vpost($base, $i);
      if ($raw !== null) {
        update_post_meta($variation_id, $db->full_meta_key($key), wc_format_decimal($raw));
      }
    }
    $raw = $this->vpost('_mns_navasan_plus_formula_id', $i);
    if ($raw !== null) {
      update_post_meta($variation_id, $db->full_meta_key('formula_id'), absint($raw));
    }
    $vars_payload =
      $_POST['_mns_navasan_plus_formula_variables'] ??
      ($_POST['_variable_mns_navasan_plus_formula_variables'] ?? null);
    if (is_array($vars_payload)) {
      ...
      foreach ($vars_payload as $fid => $codes) {
        ...
        foreach ((array) $codes as $code => $vals) {
          $code = sanitize_key($code);
          $reg = is_array($vals['regular'] ?? null)
            ? $vals['regular'][$i] ?? ''
            : $vals['regular'] ?? '';
          $sal = is_array($vals['sale'] ?? null) ? $vals['sale'][$i] ?? '' : $vals['sale'] ?? '';
          $vars[$fid][$code] = [
            'regular' => sanitize_text_field(wp_unslash($reg)),
            'sale' => sanitize_text_field(wp_unslash($sal)),
          ];
        }
      }
    ...
    foreach ($vars_meta as $code => $row) {
      ...
      if (in_array($role, ['weight', 'profit', 'charge'])) {
        ...
        if (isset($overrides[$code]['regular']) && $overrides[$code]['regular'] !== '') {
          ...
          isset($overrides[$code]) &&
          !is_array($overrides[$code]) &&
          $overrides[$code] !== ''
        ) {
        ...
        if ($value !== null && $value !== '') {
          ...
          if ($role === 'weight') {
            $formatted_value =
              number_format_i18n((float) $value, 3) .
              ' ' .
              ($symbol ?: __('grams', 'mns-navasan-plus'));
          } else {
  ...
  public function display_order_item_breakdown($item_id, $item, $product): void {
    ...
    foreach ($display_data as $role => $data) {
      ...
      echo '<td style="padding: 3px 5px; font-weight: 600; width: 35%; text-align: right; border-right: 1px solid #ddd;">' .
        esc_html($data['label']) .
        ':</td>';
      echo '<td style="padding: 3px 5px; text-align: right; color: #333;">' .
        esc_html($data['value']) .
        '</td>';
      echo '</tr>';
    }
