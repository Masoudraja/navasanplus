...
final class CategoryDiscountFields {
  ...
  public function add_fields($taxonomy): void {
    wp_nonce_field('mnsnp_cat_discount_save', '_mnsnp_cat_nonce'); ?>
        <div style="margin: 1.5em 0; border-top: 1px solid #c3c4c7; padding-top: 1.5em;">
            ...
            <p><?php esc_html_e(
              'These discounts will be applied to products in this category upon creation.',
              'mns-navasan-plus',
            ); ?></p>
            <?php foreach ($this->fields as $key => $label): ?>
                <div class="form-field term-<?php echo esc_attr($key); ?>-wrap">
                    <label for="mnsnp_<?php echo esc_attr($key); ?>"><?php echo esc_html(
  $label,
); ?></label>
                    <input type="number" step="1" name="mnsnp_discount[<?php echo esc_attr(
                      $key,
                    ); ?>]" id="mnsnp_<?php echo esc_attr($key); ?>" value="">
                </div>
            <?php endforeach; ?>
            <?php do_action('mnsnp_category_discount_add_fields', $taxonomy); ?>
        </div>
        <?php
  }
  ...
  public function edit_fields(\WP_Term $term): void {
    ...
    $clear_url = wp_nonce_url(
      admin_url(
        'admin.php?action=mnsnp_clear_cat_discounts&term_id=' .
          $term->term_id .
          '&taxonomy=' .
          $term->taxonomy,
      ),
            ...
            <td colspan="2" style="padding: 0;">
                <div id="mnsnp-category-discounts-metabox" class="postbox" style="margin-top: 1em;">
                    <div class="postbox-header">
                        <h2 class="hndle" style="cursor: default;"><?php esc_html_e(
                          'Navasan Plus - Bulk Edit Discounts',
                          'mns-navasan-plus',
                        ); ?></h2>
                    </div>
                    <div class="inside">
                        <p class="description" style="margin-bottom: 1em;"><?php esc_html_e(
                          'IMPORTANT: When you update the category, these values will be applied to ALL products in this category, overwriting their current discounts and recalculating their final prices.',
                          'mns-navasan-plus',
                        ); ?></p>
                            ...
                            <?php foreach ($this->fields as $key => $label):
                              $value = get_term_meta($term->term_id, $key, true); ?>
                                <tr class="form-field term-<?php echo esc_attr($key); ?>-wrap">
                                    <th scope="row"><label for="mnsnp_<?php echo esc_attr(
                                      $key,
                                    ); ?>"><?php echo esc_html($label); ?></label></th>
                                    <td><input type="number" step="1" name="mnsnp_discount[<?php echo esc_attr(
                                      $key,
                                    ); ?>]" id="mnsnp_<?php echo esc_attr(
  $key,
                            ...
                            <tr class="form-field">
                                <th scope="row"><?php esc_html_e(
                                  'Clear Discounts Action',
                                  'mns-navasan-plus',
                                ); ?></th>
                                <td>
                                    <a href="<?php echo esc_url(
                                      $clear_url,
                                    ); ?>" class="button button-secondary" onclick="return confirm('<?php esc_attr_e('Are you sure you want to clear these discounts for the category AND all its products?', 'mns-navasan-plus'); ?>');">
                                        <?php esc_html_e(
                                          'Clear All Discounts for This Category',
                                          'mns-navasan-plus',
                                        ); ?>
                                    </a>
                                    <p class="description"><?php esc_html_e(
                                      'This will remove the saved discounts from this category and all products within it.',
                                      'mns-navasan-plus',
                                    ); ?></p>
                                </td>
                            </tr>
                            <?php do_action('mnsnp_category_discount_edit_fields', $term); ?>
                            </tbody>
                        </table>
                    </div>
                </div>
  ...
  public function save_and_bulk_update_fields(int $term_id): void {
    if (
      !isset($_POST['_mnsnp_cat_nonce']) ||
      !wp_verify_nonce($_POST['_mnsnp_cat_nonce'], 'mnsnp_cat_discount_save')
    ) {
      return;
    }
    if (empty($_POST['mnsnp_discount']) || !is_array($_POST['mnsnp_discount'])) {
      return;
    }
    $discounts = wp_unslash($_POST['mnsnp_discount']);
    do_action('mnsnp_save_category_discount_fields', $term_id);
  ...
  public function handle_clear_discounts(): void {
    ...
    if (
      !$term_id ||
      !check_admin_referer('mnsnp_clear_cat_discounts_' . $term_id) ||
  ...
  private function apply_and_save(int $term_id, array $discounts): void {
    ...
    foreach ($this->fields as $key => $label) {
      $value =
        isset($discounts[$key]) && $discounts[$key] !== ''
          ? wc_format_decimal($discounts[$key], 0)
          : '';
