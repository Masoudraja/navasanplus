...
final class Loader {
    ...
    public function register_public_assets(): void {
        ...
        $public_css_rel = $use_min && file_exists( $plugin_dir . '/assets/css/public.min.css' ) ? 'assets/css/public.min.css' : 'assets/css/public.css';
        wp_register_style('mns-navasan-plus-public', Helpers::plugin_url( $public_css_rel ), [], $this->ver_for( $plugin_dir . '/' . $public_css_rel ));
        $parser_rel = $use_min && file_exists( $plugin_dir . '/assets/js/formula-parser.min.js' ) ? 'assets/js/formula-parser.min.js' : 'assets/js/formula-parser.js';
        wp_register_script('mns-navasan-plus-formula-parser', Helpers::plugin_url( $parser_rel ), [], $this->ver_for( $plugin_dir . '/' . $parser_rel ), true);
        $common_rel = $use_min && file_exists( $plugin_dir . '/assets/js/common.min.js' ) ? 'assets/js/common.min.js' : 'assets/js/common.js';
        wp_register_script('mns-navasan-plus-common', Helpers::plugin_url( $common_rel ), [ 'jquery' ], $this->ver_for( $plugin_dir . '/' . $common_rel ), true);
        $public_rel = $use_min && file_exists( $plugin_dir . '/assets/js/public.min.js' ) ? 'assets/js/public.min.js' : 'assets/js/public.js';
        wp_register_script('mns-navasan-plus-public', Helpers::plugin_url( $public_rel ), [ 'jquery', 'mns-navasan-plus-formula-parser' ], $this->ver_for( $plugin_dir . '/' . $public_rel ), true);
    }

    public function register_admin_assets(): void {
        $plugin_dir = dirname( __DIR__ );
        $use_min    = ! ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG );
        $admin_css_rel = $use_min && file_exists( $plugin_dir . '/assets/css/admin.min.css' ) ? 'assets/css/admin.min.css' : 'assets/css/admin.css';
        wp_register_style('mns-navasan-plus-admin', Helpers::plugin_url( $admin_css_rel ), [], $this->ver_for( $plugin_dir . '/' . $admin_css_rel ));
        ...
        $persist_rel = $use_min && file_exists( $plugin_dir . '/assets/js/persist.min.js' ) ? 'assets/js/persist.min.js' : 'assets/js/persist.js';
        ...
        $common_rel = $use_min && file_exists( $plugin_dir . '/assets/js/common.min.js' ) ? 'assets/js/common.min.js' : 'assets/js/common.js';
        wp_register_script('mns-navasan-plus-common', Helpers::plugin_url( $common_rel ), [ 'jquery' ], $this->ver_for( $plugin_dir . '/' . $common_rel ), true);
        $admin_rel = $use_min && file_exists( $plugin_dir . '/assets/js/admin.min.js' ) ? 'assets/js/admin.min.js' : 'assets/js/admin.js';
    ...
    private function boot_admin(): void {
        ...
        if ( ! class_exists( __NAMESPACE__ . '\\Admin\\CategoryDiscountFields' ) && file_exists( $cat_discount_file ) ) require_once $cat_discount_file;
    ...
    private function boot_public(): void {
        ...
        if ( ! class_exists( __NAMESPACE__ . '\\PublicNS\\ConditionalDiscounts' ) && file_exists( $conditional_discounts_file ) ) {
