...
<div class="<?php echo esc_attr( $section_class ); ?>"<?php echo $data_loop; ?>>
  ...
  $product->get_meta( '_mns_navasan_plus_active', true ),
  __( 'Rate Based', 'mns-navasan-plus' ),
  __( 'Enable rate‐based pricing for this product.', 'mns-navasan-plus' ),
  [
    'wrapper_class' => $wrapper_class . ' form-row-full mnsnp-checkbox', // ⬅️ افزودیم
    'class'         => $field_class,
  ]
);

  // 2) هشدار تلگرام (اختیاری)
//   Fields::checkbox(
//       'mns_navasan_plus_price_alert' . $id_suffix,
//       $name_prefix . '_mns_navasan_plus_price_alert' . $name_suffix,
//       $price_alert,
//       __( 'Price Alert', 'mns-navasan-plus' ),
//       __( 'Send Telegram alert when price changes.', 'mns-navasan-plus' ),
//       [ 'wrapper_class' => $wrapper_class . ' form-row-full show_if_mns_navasan_plus_active', 'class' => $field_class ]
//   );
...
<?php
// -------------------------------------------------------------------------
// پیش‌نمایش «اجزای فرمول» فقط در ادمین (برای کمک به تنظیمات؛ در فرانت نمایش ندارد)
// قابل خاموش‌کردن با فیلتر: mnsnp/show_admin_formula_breakdown
// -------------------------------------------------------------------------
...
if (
    ...
    if ( class_exists( '\MNS\NavasanPlus\Services\PriceCalculator' ) ) {
        try {
            ...
            if ( is_array( $res ) ) {
                $final = isset( $res['price'] )
                    ? (float) $res['price']
                    : ( (float) ( $res['profit'] ?? 0 ) + (float) ( $res['charge'] ?? 0 ) );
            } else {
                $final = (float) $res;
            }
            echo esc_html( number_format_i18n( max( 0, (float) $final ), 2 ) );
        } catch ( \Throwable $e ) {
            echo esc_html__( '— error —', 'mns-navasan-plus' );
        }
    } else {
    ...
    echo '</div>'; // .mnsnp-admin-preview-wrap
}