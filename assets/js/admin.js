jQuery(function ($) {
  ...
  // Helpers
  // ---------------------------------------
  function getVarsWrap() { return $('#mns-navasan-plus-formula-variables-container'); }
  function getCompsWrap() { return $('#mns-navasan-plus-formula-components-container'); }
  function getCurrencies() {
    const $wrap = getVarsWrap();
    const data = $wrap.data('currencies');
    if (Array.isArray(data)) return data;
    try { return JSON.parse(String(data || '[]')); } catch (e) { return []; }
  }

  function getRowCode($row) {
    var code = $row.attr('data-code');
    if (code) return code;
    var $any = $row.find('[name*="_mns_navasan_plus_formula_variables["]').first();
    if ($any.length) {
      var m = ($any.attr('name') || '').match(/_mns_navasan_plus_formula_variables\[([^\]]+)\]/);
      if (m) return m[1];
    }
    return 'v_0';
  }
  ...
  function replaceCodeInRow($row, oldCode, newCode) {
    ...
    $row.find('[name],[id]').each(function () {
      ...
      if (nm) {
        // [oldCode] → [newCode]
        nm = nm.replace(new RegExp('\\[' + rxEscape(oldCode) + '\\]', 'g'), '[' + newCode + ']');
        $el.attr('name', nm);
      }
      var id = $el.attr('id');
      if (id) {
        // _oldCode_ → _newCode_
        id = id.replace(new RegExp('_' + rxEscape(oldCode) + '_', 'g'), '_' + newCode + '_');
        // [oldCode] → [newCode] (برای idهایی که مثل name براکت دارند)
        id = id.replace(new RegExp('\\[' + rxEscape(oldCode) + '\\]', 'g'), '[' + newCode + ']');
        $el.attr('id', id);
      }
    });

    $row.find('.mns-var-code').text(newCode);
    $row.find('.mns-copy-code, .mns-insert-code')
  ...
  // تولید کُد یکتا
  function makeCode() {
    return 'v_' + (Date.now().toString(36) + Math.floor(Math.random() * 999).toString(36));
  }

  // ست‌کردن/یکتا کردن data-code برای همه ردیف‌های موجود
  function initRowCodes() {
    var seen = new Set();
    getVarsWrap().children('.mns-formula-variable').each(function () {
      var $row = $(this);
  ...
  function refreshVarRowUI($row) {
    ...
    if (isCurrency) {
      ...
      $unitSym.attr('readonly', 'readonly');
      ...
      if ($rateText.length) {
        var txt = (!isNaN(rate) && isFinite(rate)) ? rate.toLocaleString(undefined, { maximumFractionDigits: 4 }) : '—';
        if (sym) txt += ' ' + sym;
        $rateText.text(txt);
      }
    } else {
  ...
  function addVarRow(kind, preset) {
    ...
    if (kind === 'currency' && preset) {
      ...
      if ($cid.length && preset.currency_id) $cid.val(String(preset.currency_id));
      if (preset.name) $clone.find('input[id$="_name"]').val(preset.name);

      if (typeof preset.unit !== 'undefined') {
        $clone.find('input[id$="_unit"]').val(preset.unit).attr('readonly', 'readonly');
      }
      if (preset.unit_symbol) {
        $clone.find('input[id$="_unit_symbol"]').val(preset.unit_symbol).attr('readonly', 'readonly');
      }
  ...
  function openCurrencyPicker(onPick) {
    ...
    if (!currencies.length) { alert('No currencies found.'); return; }
    var html = `
...
<div class="mnsnp-modal" style="position:fixed;left:50%;top:30%;transform:translateX(-50%);background:#fff;border:1px solid #ccd0d4;border-radius:6px;box-shadow:0 10px 30px rgba(0,0,0,.2);width:420px;z-index:10000;">
  ...
  <div style="padding:16px;">
    ...
    <select class="mnsnp-curr-select" style="width:100%;">
      ...
      ${currencies.map(c => `<option value="${c.id}" data-rate="${c.rate}" data-symbol="${c.symbol || ''}">${(c.label || ('#' + c.id))}</option>`).join('')}
    </select>
  </div>
  <div style="padding:12px 16px;border-top:1px solid #eee;text-align:left;">
    ...
    var $wrap = $('<div class="mnsnp-curr-picker-wrap"></div>').appendTo(document.body).html(html);
    $wrap.on('click', '.mnsnp-curr-cancel, .mnsnp-modal-backdrop', function (e) { e.preventDefault(); $wrap.remove(); });
  ...
  // تغییر نوع و ارز
  $(document).on('change', '#mns-navasan-plus-formula-variables-container select[id$="_type"]', function () {
    refreshVarRowUI($(this).closest('.mns-formula-variable'));
    triggerRecalcAll();
  });
  $(document).on('change', '#mns-navasan-plus-formula-variables-container select[id$="_currency_id"]', function () {
  ...
  function insertAtCaret($ta, text) {
    ...
    if (typeof el.selectionStart === 'number') {
      ...
      el.focus(); var range = document.selection.createRange(); range.text = text;
    } else {
      el.value += text;
    }
  ...
  function copyText(text) {
    ...
    var $t = $('<textarea style="position:fixed;left:-9999px;top:-9999px;"></textarea>').val(text).appendTo('body');
  ...
  // ---------------------------------------
  var parser = (window.FormulaParser && FormulaParser.Parser) ? new FormulaParser.Parser() : null;
  ...
  function recalcComponents() {
    ...
    getCompsWrap().children('.mns-formula-component').each(function () {
      var $row = $(this);
      var $exp = $row.find('.mns-component-expression, [id$="_expression"], [name$="[expression]"]').first();
      var $out = $row.find('.mns-component-total').first();
      if (!$exp.length || !$out.length) return;
      var v = evalExpr($exp.val(), vars);
      $out.text(v === null ? '—' : v.toLocaleString(undefined, { maximumFractionDigits: 4 }));
      ...
      if ($symSpan.length) $symSpan.text($symInput.length && $symInput.val() ? (' ' + $symInput.val()) : '');
    });
  }
  ...
  function recalcFormulaLiveRow() {
    ...
    try {
      ...
      if (typeof result === 'number' && isFinite(result)) {
        $out.text(result.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        if ($err.length) $err.hide().text('');
      } else {
        $out.text('—');
      }
      ...
      $out.text('—');
      if ($err.length) $err.text(e && e.message ? e.message : 'Parse error').show();
    }
  }
  ...
  function recalcGlobalFallback() {
    var vars = buildVarsMap();
    var $exp = $('.mns-formula-expression textarea, [id$="_formula_expression"], [name$="[formula_expression]"]').first();
  ...
  $(document).on('input change', '#mns-navasan-plus-formula-variables-container input, #mns-navasan-plus-formula-variables-container select, #mns-navasan-plus-formula-variables-container textarea', triggerRecalcAll);

  // تغییر روی اکسپرشن هر کامپوننت
  $(document).on('input change', '#mns-navasan-plus-formula-components-container .mns-component-expression, #mns-navasan-plus-formula-components-container [id$="_expression"], #mns-navasan-plus-formula-components-container [name$="[expression]"]', triggerRecalcAll);

  // تغییر روی اکسپرشن کل
  $(document).on('input change', '#mns_navasan_plus_formula_expression', triggerRecalcAll);
  $(document).on('input change', '.mns-formula-expression textarea, [id$="_formula_expression"], [name$="[formula_expression]"]', triggerRecalcAll);
  ...
  initRowCodes();
  getVarsWrap().children('.mns-formula-variable').each(function(){ refreshVarRowUI($(this)); });
  triggerRecalcAll();
});